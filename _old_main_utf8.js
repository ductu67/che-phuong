document.addEventListener('DOMContentLoaded', () => {
    ...
    const availableToppings = [
        ...
        { name: 'Tr├ón ch├óu ─æen', icon: 'ph-circle', price: 5000 },
        { name: 'Tr├ón ch├óu gi├▓n', icon: 'ph-circle-dashed', price: 5000 },
        { name: 'Tr├ón ch├óu nh├ón dß╗½a', icon: 'ph-nut', price: 8000 }
    ];
    ...
    function renderCart() {
        if (cart.length === 0) {
            cartItemsContainer.innerHTML = '<p class="empty-cart-msg">Giß╗Å h├áng ─æang trß╗æng. H├úy chß╗ìn m├│n thß║ú ga nh├⌐!</p>';
            ...
            cart.forEach((item, index) => {
                ...
                div.innerHTML = `
                    ...
                    <div class="cart-item-total">
                        ${totalForItem.toLocaleString('vi-VN')}─æ
                    </div>
                `;
            ...
            if (!document.getElementById('clear-cart-btn')) {
                ...
                clearBtn.textContent = 'X├│a to├án bß╗Ö giß╗Å h├áng';
                clearBtn.addEventListener('click', () => {
                    if (confirm('Bß║ín c├│ chß║»c chß║»n muß╗æn x├│a to├án bß╗Ö giß╗Å h├áng?')) {
                        cart = [];
                        renderCart();
                        showToast('─É├ú x├│a giß╗Å h├áng');
                    }
                });
        ...
        if (grandTotalEl) {
            ...
            grandTotalEl.textContent = grandTotal.toLocaleString('vi-VN') + '─æ';
        }
    ...
    document.querySelectorAll('.card').forEach(card => {
        card.addEventListener('click', (e) => {
            ...
            const priceEl = card.querySelector('.card-price') || card.querySelector('.new-price');
            const price = priceEl ? priceEl.textContent : '0─æ';
            ...
            const catName = parentSection ? parentSection.querySelector('h2').textContent : "Phß╗ò biß║┐n";
            ...
            // Toppings Logic
            const needsTopping = name.includes('Ch├¿') || name.includes('Sß╗»a Chua') || name.includes('S╞░╞íng S├ío') || name.includes('Tr├á') || name.includes('Matcha') || name.includes('─Éß╗ô Uß╗æng');
    ...
    function addToCart(name, selectedToppings = [], price = 0) {
        ...
        showToast(`─É├ú th├¬m ${name} v├áo giß╗Å!`);
    }
    ...
    checkoutBtn.addEventListener('click', () => {
        ...
        let orderText = "Ch├áo tiß╗çm Ch├¿ Ph╞░╞íng, m├¼nh muß╗æn ─æß║╖t c├íc m├│n sau (├¡t ngß╗ìt, giao tß║¡n n╞íi ß║í):\n";
        let grandTotal = 0;
        cart.forEach(item => {
            const itemPrice = item.price || 0;
            const toppingPrice = (item.selectedToppings || []).reduce((sum, t) => sum + (t.price || 0), 0);
            ...
            orderText += `: ${totalForItem.toLocaleString('vi-VN')}─æ\n`;
        });
        orderText += `\nTß╗òng cß╗Öng: ${grandTotal.toLocaleString('vi-VN')}─æ`;
        ...
        const handleSuccess = () => {
            showToast('─É├ú ch├⌐p ─æ╞ín. Mß╗ƒ Messenger...');
        ...
        const handleFailure = (err) => {
            ...
            showToast('Lß╗ùi ch├⌐p ─æ╞ín. ─Éang mß╗ƒ Messenger...');
            window.location.href = 'https://m.me/phuong.nguyen.298061';
        };
    ...
    const messages = [
        'Mß╗Öt kh├ích vß╗½a ─æß║╖t 2 Combo Sß╗æng Thß║únh Th╞íi.',
        '15 ng╞░ß╗¥i ─æang xem thß╗▒c ─æ╞ín cß╗ºa tiß╗çm.',
        'M├│n Ch├¿ Sß║ºu Ri├¬ng ─æang rß║Ñt hot h├┤m nay!',
        'Mß╗Öt kh├ích vß╗½a ─æß║╖t Ch├¿ B╞░ß╗ƒi ├¡t ngß╗ìt.',
        '8 ng╞░ß╗¥i ─æang lß╗▒a chß╗ìn m├│n cho bß╗»a xß║┐.'
    ];
    ...
    function updateShopStatus() {
        ...
        if (hour >= 8 && hour < 22) {
            badge.textContent = '─Éang mß╗ƒ cß╗¡a';
            badge.className = 'status-badge status-open';
        } else {
            badge.textContent = 'Qu├ín ─æ├ú nghß╗ë';
